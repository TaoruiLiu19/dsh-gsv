...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const HELP_DOC = [
			...
			{ title: "三、常见问题", steps: [
				"· 引擎未启动：打开 设置 → 声音设置 的开关启动（模型加载约 15~90 秒）",
		...
		function HelpSection() {
			...
			return jsx.jsx("div", { children: HELP_DOC.map((sec) => jsx.jsxs("div", { style: blockStyle, children: [
		...
		function apply(ctx) {
			...
			// 引擎设置项（与"模型""插件"同级）：引擎开关 + 声音设置（TTS 配置）
